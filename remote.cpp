...
#include <infrared/lib/infrared/infrared_signal.h>
#include <core/log.h>
...
// Adresa telecomenzii Vortex (protocol RCA, 8 biti)
static const uint8_t rca_address = 0x0F;

// Codurile de comanda RCA (8 biti fiecare, nu deplasate pe 32 de biti)
static const uint8_t rca_commands[] = {
    0x54, 0xC5, 0xFC, // Power, Source, Mute
    0xF4, 0xB4, 0x10, // Vol+, Ch+, Menu
    0x74, 0x34, 0x60, // Vol-, Ch-, Exit
    0x9A, 0x2F, 0x6A, // Up, Ok, Left
    0x1A, 0xEA, 0xE4  // Down, Right, Back
};

#define BUTTON_COUNT (sizeof(rca_commands) / sizeof(rca_commands[0]))

// Randarea interfetei grafice
...
// Trimiterea reala a semnalului IR folosind API-ul public InfraredSignal
static void send_ir_signal(NotificationApp* notifications, uint8_t command) {
    ...
    FURI_LOG_I(TAG, "Sending IR RCA: Addr=0x%02X Cmd=0x%02X", rca_address, command);

    InfraredSignal* signal = infrared_signal_alloc();
    if(!signal) {
        FURI_LOG_E(TAG, "Failed to allocate InfraredSignal");
        return;
    }
    ...
    if(infrared_signal_is_valid(signal)) {
        // Feedback vizual + haptic la trimitere
        ...
        FURI_LOG_E(TAG, "Invalid IR signal, not sending");
    }
    ...
    while(running &&
        ...
        if(event.type == InputTypeShort || event.type == InputTypeRepeat) {
            ...
            case InputKeyUp:
                ...
                break;
            case InputKeyDown:
                if(app->selected_button <= (int)BUTTON_COUNT - 4) app->selected_button += 3;
                break;
            case InputKeyLeft:
            ...
            case InputKeyOk:
                if(event.type == InputTypeShort) {
                    send_ir_signal(app->notifications, rca_commands[app->selected_button]);
                }
                break;
            default:
                break;
